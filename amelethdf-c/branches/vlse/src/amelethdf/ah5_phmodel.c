...
void AH5_read_phm_volume_instance (hid_t file_id, const char *path, AH5_volume_instance_t *volume_instance)
{
    ...
    AH5_read_floatingtype(file_id, path2, &(volume_instance->relative_permittivity));
    strcpy(path2, path);
    strcat(path2, AH5_G_RELATIVE_PERMEABILITY);
    AH5_read_floatingtype(file_id, path2, &(volume_instance->relative_permeability));
    strcpy(path2, path);
    strcat(path2, AH5_G_ELECTRIAH5_C_CONDUCTIVITY);
    AH5_read_floatingtype(file_id, path2, &(volume_instance->electric_conductivity));
    strcpy(path2, path);
    strcat(path2, AH5_G_MAGNETIAH5_C_CONDUCTIVITY);
    AH5_read_floatingtype(file_id, path2, &(volume_instance->magnetic_conductivity));
}
...
{
    char mandatory[][AH5_ATTR_LENGTH] = {AH5_A_ZS};
...
{
    char mandatory[][AH5_ATTR_LENGTH] = {AH5_A_ZS, AH5_A_ZT};
...
void AH5_read_phm_surface_instance (hid_t file_id, const char *path, AH5_surface_instance_t *surface_instance)
{
    ...
    {
        if (strcmp(temp, AH5_V_THIN_DIELECTRIAH5_C_LAYER) == 0)
            AH5_read_phm_surface_instance_tdl(file_id, path, surface_instance);
        else if (strcmp(temp, AH5_V_SIBC) == 0)
            AH5_read_phm_surface_instance_sibc(file_id, path, surface_instance);
        else if (strcmp(temp, AH5_V_ZS) == 0)
            AH5_read_phm_surface_instance_zs(file_id, path, surface_instance);
        else if (strcmp(temp, AH5_V_ZSZT) == 0)
            AH5_read_phm_surface_instance_zszt(file_id, path, surface_instance);
        else if (strcmp(temp, AH5_V_ZSZT2) == 0)
            AH5_read_phm_surface_instance_zszt2(file_id, path, surface_instance);
        else
        {
...
void AH5_read_physicalmodel (hid_t file_id, AH5_physicalmodel_t *physicalmodel)
{
    ...
    if (children.nb_children > 0)
    {
        ...
        for (i = 0; i < children.nb_children; i++)
        {
            ...
            AH5_read_phm_volume_instance(file_id, path, physicalmodel->volume_instances + i);
    ...
    if (children.nb_children > 0)
    {
        ...
        for (i = 0; i < children.nb_children; i++)
        {
            ...
            strcat(path, AH5_G_SURFACE);
            strcat(path, children.childnames[i]);
            AH5_read_phm_surface_instance(file_id, path, physicalmodel->surface_instances + i);
            free(children.childnames[i]);
        }
        free(children.childnames);
    }
    ...
    if (children.nb_children > 0)
    {
        ...
        for (i = 0; i < children.nb_children; i++)
        {
            ...
            strcat(path, AH5_G_INTERFACE);
            strcat(path, children.childnames[i]);
            AH5_read_phm_interface_instance(file_id, path, physicalmodel->interface_instances + i);
            free(children.childnames[i]);
        }
        free(children.childnames);
    }

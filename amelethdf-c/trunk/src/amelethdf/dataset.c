...
dataset_t read_dataset(hid_t loc_id,const char* path)
{
	...
	if(status<0) printf("can't read rank for %s\n",path);
	dims = (hsize_t *) malloc( rank * sizeof(hsize_t));
	type_class=(H5T_class_t *)malloc(sizeof(H5T_class_t));
	status = H5LTget_dataset_info(loc_id,path,dims,type_class,NULL);
	if(status<0) printf("can't read dims for %s\n",path);
	length=dims[0];
	ds.dims = (hsize_t *) malloc( rank * sizeof(hsize_t));
	for(i=0;i<rank;i++) ds.dims[rank-i-1] = dims[i];
	for(i=1;i<rank;i++) length=length*dims[i];
	ds.nbdims = rank;
	ds.ivalue=NULL;
	ds.rvalue=NULL;
	ds.cvalue=NULL;
...
dataset_t clear_content_dataset(dataset_t dataset)
{
	...
	if(ds.dims!=NULL) free(ds.dims);
	if(ds.ivalue!=NULL) free(ds.ivalue);
	if(ds.rvalue!=NULL) free(ds.rvalue);
	if(ds.cvalue!=NULL) free(ds.cvalue);
	return ds;
}

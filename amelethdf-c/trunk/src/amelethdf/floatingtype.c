...
int get_type_ft(hid_t loc_id, const char* path)
{
    ...
    if(strcmp(buf,V_SINGLE_REAL)==0) type=E_SINGLE_REAL;
    else if (strcmp(buf,V_SINGLE_COMPLEX)==0) type=E_SINGLE_COMPLEX;
    else if (strcmp(buf,V_DATA_SET)==0) type=E_DATA_SET;
    else if (strcmp(buf,V_VECTOR)==0) type=E_VECTOR;
    else if (strcmp(buf,V_ARRAY_SET)==0) type=E_ARRAY_SET;
    else printf("Can't get floating type for %s\n",path);
    free(buf);
    return type;
}
...
int issinglereal(floatingtype_t ft)
{
	...
	if(ft.floatingtype == E_SINGLE_REAL) singlereal=TRUE;
	return singlereal;
}
...
int issinglecomplex(floatingtype_t ft)
{
	...
	if(ft.floatingtype == E_SINGLE_COMPLEX) singlecomplex=TRUE;
...
{
	int vect = FALSE;
	if(ft.floatingtype == E_VECTOR) vect=TRUE;
	return vect;
}
...
{
	int ds = FALSE;
	if(ft.floatingtype == E_DATA_SET) ds=TRUE;
	return ds;
}
...
{
	int ars = FALSE;
	if(ft.floatingtype == E_ARRAY_SET) ars=TRUE;
	return ars;
}

...
H5I_type_t gettype(hid_t loc_id, char* path)
{
	...
	if(type == H5I_BADID) printf("Not valid type for %s\n",path);
	status = closetype(obj_id);
	if (status<0) printf("Can't close type id for %s\n",path);
	return type;
}
...
herr_t closetype(hid_t obj_id)
{
	...
	status = -1;
	if(object_type == H5I_DATASET) status=H5Dclose(obj_id);
	else if(object_type == H5I_GROUP) status=H5Gclose(obj_id);
	return status;
}
...
{
	int isleaf=FALSE;
	if(gettype(loc_id,path) == H5I_DATASET) isleaf = TRUE;
	return isleaf;
}
...
{
	int isgroup = FALSE;
	if(gettype(loc_id,path) == H5I_GROUP) isgroup = TRUE;
	return isgroup;
}
...
char * remove_sep(char *path)
{
	...
	int i;
	char *pch;
	if (path_isabs(path)) begin=1;
	int pathsize=strlen(path);
	buf=(char *)malloc(pathsize*sizeof(char));
	pch=strchr(path,'/');
	while(pch!=NULL)
	{
		if(strcmp(pch,"/")==0) end=1;
		pch=strchr(pch+1,'/');
	}
...
int path_contain(char *path, char *subpath)
{
	...
	int contain;
	contain = FALSE;
	if(strcmp(path,subpath)>0) contain = TRUE;
	return contain;
}
...
char * path_element(char *path, int index, int back)
{
	...
	if(strcmp(path,"")==0) strcpy(res,path);
	else
	{
		strbuf = path_split(path);
		if (index>strbuf.nb) strcpy(res,"");
		else
		{
			if(back == FALSE)
				{
...
int path_like(char * path, char* pattern)
{
	...
	for(i=0;i<buf2.nb;i++)
		{
		if(strcmp(buf2.chartab[i],"*")!=0) nb_inf=i;
		}
	nb_sup=buf2.nb-nb_inf;
	for(i=0;i<nb_inf;i++)
	{
		if(strcmp(buf2.chartab[i],buf1.chartab[i])!=0) res_inf = FALSE;
	}
	if(res_inf==FALSE) res=FALSE;
	else
	{
		if(nb_sup>0)
		{
			for(i=0;i>nb_sup;i++)
			{
				if(strcmp(buf1.chartab[buf1.nb-i],buf2.chartab[buf2.nb-i])!=0)
					      res_sup=FALSE;
			}
		}
		if((res_inf==TRUE) && (res_sup==TRUE)) res=TRUE;
	}
	return res;
}

...
planewave_t read_planewave(hid_t file_id, const char* path)
{
	...
	status = H5Aexists_by_name(file_id,path,A_LINEAR_POLARIZATION,H5P_DEFAULT);
	if(status<0)
	{
		pw.elliptical_polarization_etheta=read_complex_attribute(file_id,path,
	...
	{
		pw.linear_polarization = read_float_attribute(file_id,path,A_LINEAR_POLARIZATION);
	}
...
int islinear(planewave_t pw)
{
	int linear;
	linear = FALSE;
	if(pw.polarization==LINEAR) linear=TRUE;
	return linear;
}
...
int iselliptic(planewave_t pw)
{
	int elliptic;
	elliptic = FALSE;
	if(pw.polarization==ELLIPTIC) elliptic=TRUE;
	return elliptic;
}

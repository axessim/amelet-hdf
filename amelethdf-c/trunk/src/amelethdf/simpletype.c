...
single_t read_single(hid_t loc_id, const char* path)
{
	...
	single.physical_nature = (char*)malloc(ABSOLUTE_PATH_NAME_LENGTH*sizeof(char*));
	strcpy(single.physical_nature,"");
	test = H5Aexists_by_name(loc_id,path,A_PHYSICAL_NATURE,H5P_DEFAULT);
	if(test>0)
	{
		strcpy(single.physical_nature,read_string_attribute(loc_id,path,A_PHYSICAL_NATURE));
	}
...
char* singlecomplex_to_string(singlecomplex_t singlecomplex)
{
	...
	sprintf(buf," %f + i %f ",crealf(singlecomplex.value),cimagf(singlecomplex.value));
	strcat(s," , Value : ");
	strcat(s, buf);
	return s;
}
...
herr_t make_single(hid_t loc_id, const char* path, single_t single)
{
	...
	status = H5LTset_attribute_string(loc_id,path,A_PHYSICAL_NATURE,single.physical_nature);
	if(status<0)
	{

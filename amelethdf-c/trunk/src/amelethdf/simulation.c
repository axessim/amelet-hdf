...
simulation_t read_simulation(hid_t file_id, const char* sim_path)
{
	...
	if(H5LTfind_dataset(file_id,path)==0)
		...
		sim.inputs[0] = (char *)malloc(sim.nb_input[0]*
							             sim.nb_input[1]*sizeof(char));
	...
	if(H5LTfind_dataset(file_id,path)==0)
	{
		...
		sim.outputs[0] = (char *)malloc(sim.nb_output[0]
                                        *sim.nb_output[1]*sizeof(char));
...
int* read_nb_inputs_outputs(hid_t file_id,const char* path)
{
	...
	if(status<0) printf("can't read the number of inputs for %s\n",path);

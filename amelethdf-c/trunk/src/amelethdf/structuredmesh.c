...
axis_t readAxis(hid_t loc_id,const char *axisname)
{
	  ...
	  status = H5Dread(axis_id, H5T_NATIVE_FLOAT, H5S_ALL, H5S_ALL, H5P_DEFAULT, rdata.nodes);
	  return rdata;
}
...
snorm_t readNormals(hid_t loc_id)
{
	...
	memtype = H5Tcopy (H5T_C_S1);
	status = H5Tset_size(memtype,sdim);
	status = H5Dread(loc_id, memtype, H5S_ALL, H5S_ALL, H5P_DEFAULT, rdata.normals[0]);
    // read attributes
    return rdata;
}

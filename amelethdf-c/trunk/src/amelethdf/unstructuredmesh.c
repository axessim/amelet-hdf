...
nodes_t readNodes(hid_t nodes_id)
{
  ...
  status = H5Dread(nodes_id, H5T_NATIVE_FLOAT, H5S_ALL, H5S_ALL, H5P_DEFAULT, rdata.nodes[0]);
  return rdata;
}
...
elttypes_t readElementTypes(hid_t eltypes_id)
{
  ...
  status = H5Dread(eltypes_id, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, rdata.elttypes);
  return rdata;
}
...
eltnodes_t readElementNodes(hid_t elnodes_id)
{
  ...
  status = H5Dread(elnodes_id, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, rdata.eltnodes);
  return rdata;
}
...
semelts_t readSemElt(hid_t file_id, const char *table_name)
{
 ...
 size_t dst_offset[5] = { HOFFSET( semelt_t, shortname ),
                          HOFFSET( semelt_t, index ),
                          HOFFSET( semelt_t, v1 ),
                          HOFFSET( semelt_t, v2 ),
                          HOFFSET( semelt_t, v3 )};

 size_t dst_sizes[5] = { sizeof( rdata.somelt[0].shortname),
                         sizeof( rdata.somelt[0].index),
                         sizeof( rdata.somelt[0].v1),
                         sizeof( rdata.somelt[0].v2),
                         sizeof( rdata.somelt[0].v3)};

 table_id = H5TBget_table_info (file_id, table_name, &nfields_out, &nrecords_out );
 ...
 // Read the table
 status=H5TBread_table( file_id, table_name, dst_size, dst_offset, dst_sizes, rdata.somelt );
 return rdata;
}
...
semnodes_t readSemNodes(hid_t file_id, const char *table_name)
{
 ...
 size_t dst_offset[2] = { HOFFSET( semnode_t, shortname ),
                          HOFFSET( semnode_t, index )};

 size_t dst_sizes[2] = { sizeof( rdata.somnodes[0].shortname),
                         sizeof( rdata.somnodes[0].index)};
 ...
 // Memory allocation for rdata before reading the table
 rdata.somnodes = (semnode_t *) malloc ((int)nrecords_out * sizeof (semnode_t));
 rdata.nbsomnodes = nrecords_out;

 // Read the table
 status=H5TBread_table( file_id, table_name, dst_size, dst_offset, dst_sizes, rdata.somnodes );
 return rdata;
}
...
ugroup_t readUGroup(hid_t group_id,const char* name)
{
  ...
  status = H5Dread(group_id, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, rdata.eltgroup);
  return rdata;
}

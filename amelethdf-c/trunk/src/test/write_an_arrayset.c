		                     ...
		                     char* comment)
{
	...
	status = H5LTset_attribute_string(file_id,path,"unit",
			unit);
	if(status < 0)
	{
		printf("Can't create attribute !!!\n");
	...
	status = H5LTset_attribute_string(file_id,path,"comment",
			comment);
	if(status < 0)
	{
		printf("Can't create attribute !!!\n");
...
char *argv[];
{
	...
	status = H5LTset_attribute_string(file_id,"/","AMELETHDF_FORMAT_VERSION","1.0.0");
	if(status < 0)
	{
		printf("Can't create attribute !!!\n");
		exit(-1);
	...
	grp_id = H5Gcreate(file_id, "floatingType",H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
	if (grp_id <0)
	{
	...
	grp_id = H5Gcreate(grp_id, "an_arrayset",H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
	if (grp_id <0)
	{
		printf("Can't create group !!!\n");
		exit(-1);
	...
	status = H5LTset_attribute_string(file_id,"floatingType/an_arrayset","floatingType","arraySet");
	if(status < 0)
	{
		printf("Can't create attribute !!!\n");
		exit(-1);
	...
	status = H5LTmake_dataset_float(file_id,"floatingType/an_arrayset/ds/dim1",1,dims1,dim1);
	if(status < 0)
	{
		printf("Can't create dataset !!!\n");
		exit(-1);
	...
	status = H5LTmake_dataset_float(file_id,"floatingType/an_arrayset/ds/dim2",1,dims2,dim2);
	if(status < 0)
	{
		printf("Can't create dataset !!!\n");
		exit(-1);

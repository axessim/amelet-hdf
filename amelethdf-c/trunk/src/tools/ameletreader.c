	...
	for(i=0;i<children.nbchild;i++)
    	{
    		...
    		for(j=0;j<children2.nbchild;j++)
    		{
    			...
    			if(meshtype(file_id,path2)==UNSTRUCTURED)
    			{
    				...
    				umesh.name=(char *)malloc(ELEMENT_NAME_LENGTH*sizeof(char));
    			...
    			else if(meshtype(file_id,path2)==STRUCTURED)
    			{
    				...
    				smesh.name=(char *)malloc(ELEMENT_NAME_LENGTH*sizeof(char));
    ...
    if (H5Lexists(file_id,C_ELECTROMAGNETIC_SOURCE,H5P_DEFAULT)!=FALSE)
    {
	  ...
	  if(strcmp(C_PLANE_WAVE,children.childnames[i])==0)
	  {
    		for(j=0;j<children2.nbchild;j++)
    		{
    			...
    			if(islinear(pw)) printf(" Linear Polarisation = %f\n",pw.linear_polarization);
    			printf("Polarization is elliptic = %i\n",iselliptic(pw));
    			if(iselliptic(pw))
    			{
    				printf(" Etheta = %f + j*%f\n",crealf(pw.elliptical_polarization_etheta),
    						cimagf(pw.elliptical_polarization_etheta));
    				printf(" Ephi = %f + j*%f\n",crealf(pw.elliptical_polarization_ephi),
    				    						cimagf(pw.elliptical_polarization_ephi));
    			}
    			printf("Magnitude : %f + i*%f\n",crealf(pw.magnitude.singlecomplex.value),
    					                        cimagf(pw.magnitude.singlecomplex.value));
    			free(path2);
    		}
    ...
    if(H5Lexists(file_id,"/externalElement",H5P_DEFAULT)!=FALSE)
    {
    	...
    	for(i=0;i<children.nbchild;i++)
    	{
    		...
    		for(j=0;j<extelt.nb_ext_el;j++)
    		{
    			...
    			printf("  %s %s %s\n",extelt.external_name[j],extelt.file_name[j],extelt.internal_name[j]);
    		}
    	}
    }
